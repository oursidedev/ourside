...
function map(row:{id:string;couple_id:string;title:string;category:BucketListItem["category"];completed_at:string|null}):BucketListItem{return{id:row.id,coupleId:row.couple_id,title:row.title,category:row.category,completed:Boolean(row.completed_at)};}
