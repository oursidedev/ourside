...
export async function getStorageSummary(){await requireAdmin();const db=await createServerSupabaseClient();const {data,error}=await db!.rpc("admin_storage_summary");if(error)throw error;return data as {totalFiles:number;totalBytes:number;images:number;videos:number;failedUploads:number};}
export async function getAdminUsers(){await requireAdmin();const db=await createServerSupabaseClient();const {data,error}=await db!.from("admin_users").select("id,email,role,is_active,created_at").order("created_at");if(error)throw error;return data||[];}
