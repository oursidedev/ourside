...
export default function NewMemory() {
  const router = useRouter(); const { plan } = useBillingPlan(); const [images, setImages] = useState<PreparedImage[]>([]); const [busy, setBusy] = useState(false); const [progress, setProgress] = useState(0); const [error, setError] = useState("");
  async function submit(event: React.FormEvent<HTMLFormElement>) {
    event.preventDefault(); if (busy) return; const form = new FormData(event.currentTarget); setBusy(true); setError(""); setProgress(1);
  ...
  return <div className="mx-auto max-w-3xl">
    ...
    <form onSubmit={submit} className="mt-8 space-y-6 rounded-[2rem] border bg-paper p-5 shadow-card sm:p-8">
      <ImageUploadDropzone plan={plan} onPreparedChange={setImages} />
