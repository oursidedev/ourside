...
function hasPrefix(pathname: string, prefixes: string[]) { return prefixes.some(prefix => pathname === prefix || pathname.startsWith(`${prefix}/`)); }
function target(base: string, request: NextRequest, pathname = request.nextUrl.pathname) { const url = new URL(`${pathname}${request.nextUrl.search}`, `${base}/`); return NextResponse.redirect(url, 308); }
...
export async function middleware(request: NextRequest) {
  ...
  if (hostname === `www.${BRAND.domain}`) return hasPrefix(pathname, APP_PREFIXES) ? target(BRAND.appUrl, request) : target(BRAND.marketingUrl, request);
  if (hostname === marketingHost && hasPrefix(pathname, APP_PREFIXES)) return target(BRAND.appUrl, request);
  if (hostname === appHost && MARKETING_PATHS.includes(pathname)) return target(BRAND.marketingUrl, request);
  if (hostname === appHost && pathname === "/") return target(BRAND.appUrl, request, "/dashboard");

  // Marketing and anonymous auth/invite routes do not need a Supabase session.
  if (!hasPrefix(pathname, PROTECTED_PREFIXES)) return pageResponse(request, privateHost);
  ...
  if (!data.user) {
    ...
    const login = new URL("/login", hostname === appHost ? BRAND.appUrl : request.url);
  ...
  if (hasPrefix(pathname, PARTNER_PREFIXES)) {
    ...
    if (!complete) {
      const dashboard = new URL("/dashboard?locked=partner", hostname === appHost ? BRAND.appUrl : request.url);
